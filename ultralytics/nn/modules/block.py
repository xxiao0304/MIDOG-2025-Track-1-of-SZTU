...
class C3k(C3):
    """_LSC3k is a CSP bottleneck module with customizable kernel sizes for feature extraction in neural networks."""
...
class C3k_LS(C3):
    """使用LSBottleneck的C3k模块，无需修改LSBottleneck"""
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = True, g: int = 1, e: float = 0.5, k: int = 3):
        ...
        Args:
            ...
            g (int): 卷积分组数
            e (float): 扩展比例
            k (int): 卷积核大小（仅用于兼容性，不影响LSBottleneck）
        """
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # 隐藏通道数
        
        # 使用LSBottleneck替换原始Bottleneck
        # 只传递LSBottleneck需要的参数，忽略k和e
        self.m = nn.Sequential(*(
            LSBottleneck(c_, c_, shortcut, g) 
            for _ in range(n)
        ))
