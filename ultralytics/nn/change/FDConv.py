...
# https://arxiv.org/pdf/2503.18783  cvpr2025
...
class KernelSpatialModulation_Global(nn.Module):
    ...
    def skip(self, x):  # 注意：此处需移除 @staticmethod 装饰器，作为实例方法
        # 返回与x同设备、同dtype的1.0张量，形状为(1,)以便后续广播
        return torch.tensor(1.0, device=x.device, dtype=x.dtype).unsqueeze(0)
...
class FDConv(nn.Conv2d):
    def __init__(self,
                 ...
                 stride=1,                   # 位置参数4（默认1）
                 padding=0,                  # 位置参数5（默认0）
                 kernel_num=8,               # 位置参数6（默认4）
                 bias=True,                  # 位置参数7（默认True）
                 reduction=0.0625,
...
#
# class FDConv2(nn.Conv2d):
#     def __init__(self,
#                  in_channels=128, out_channels=64,kernel_size=3, kernel_num=8,  padding=1, bias=True
#                  ):
#         super().__init__()
#         self.m = FDConv(in_channels, out_channels, kernel_num, kernel_size, padding, bias)
#
#     def forward(self, x):
#         x=self.m(x)
#         return x
...
if __name__ == '__main__':
    ...
    # m = FDConv(in_channels=128, out_channels=64, kernel_size=3, stride=2, padding=1,kernel_num=8,  bias=True)
    m =FDConv(3,16, 3, 2)
    # m2 = DFT_Att(n=128)
    # print(m)
    # m.convert2dftweight()
